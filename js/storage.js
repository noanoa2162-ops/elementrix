...
// ========================================
const STORAGE_KEY = 'codeComponentsStore';
const CURRENT_USER_KEY = 'currentUser';
const STORAGE_VERSION = 'v2.0'; // Version for forcing data refresh
class AppStorageManager {
    ...
    initializeDefaultData() {
        ...
        if (!this.data.users.find((u) => u.isAdmin)) {
            const admin = {
                id: this.generateId(),
                username: 'admin',
    ...
    addSampleComponents() {
        const admin = this.data.users.find((u) => u.isAdmin);
        if (!admin)
            return;
        ...
        samples.forEach((sample) => {
            const component = Object.assign(Object.assign({}, sample), { id: this.generateId(), authorId: admin.id, authorName: admin.username, status: 'approved', rating: 4.5, ratingsCount: 10, purchaseCount: 0, createdAt: Date.now(), approvedAt: Date.now() });
    ...
    getApprovedComponents() {
        return this.data.components.filter((c) => c.status === 'approved');
    }
    getPendingComponents() {
        return this.data.components.filter((c) => c.status === 'pending');
    }
    getComponentById(id) {
        return this.data.components.find((c) => c.id === id);
    }
    getComponentsByAuthor(authorId) {
        return this.data.components.filter((c) => c.authorId === authorId);
    }
    ...
    hasPurchased(userId, componentId) {
        return this.data.purchases.some((p) => p.userId === userId && p.componentId === componentId);
    }
    ...
    addPointsPurchase(pointsPurchase) {
    ...
    getRatingsByComponent(componentId) {
        return this.data.ratings.filter((r) => r.componentId === componentId);
    }
    addRating(rating) {
    ...
    updateRating(rating) {
        const index = this.data.ratings.findIndex((r) => r.id === rating.id);
        if (index !== -1) {
    ...
    // Activity operations
    getActivities() {
        return this.data.logs.sort((a, b) => b.timestamp - a.timestamp);
    }
    ...
    addLog(type, userId, message) {
        var _a;
        const activity = {
            id: this.generateId(),
            type: type,
            userId,
            username: ((_a = this.getUserById(userId)) === null || _a === void 0 ? void 0 : _a.username) || 'Unknown',
            description: message,
            timestamp: Date.now()
        };
    ...
    // Rating methods
    getUserRating(userId, componentId) {
        const rating = this.data.ratings.find((r) => r.userId === userId && r.componentId === componentId);
        return rating ? rating.rating : 0;
    }
    rateComponent(userId, componentId, rating) {
        // Check if user already rated
        const existingRatingIndex = this.data.ratings.findIndex((r) => r.userId === userId && r.componentId === componentId);
    ...
    updateComponentRating(componentId) {
        ...
        if (!component)
            return;
        const componentRatings = this.data.ratings.filter((r) => r.componentId === componentId);
        if (componentRatings.length > 0) {
            const sum = componentRatings.reduce((acc, r) => acc + r.rating, 0);
...
const storage = new AppStorageManager();
