...
interface Component {
    id: number;
    name: string;
    category: 'buttons' | 'cards' | 'forms' | 'menus' | 'animations' | 'other';
    description: string;
    price: number;
    image: string;
    rating: number;
    ...
    css?: string;
    js?: string;
    recommended?: boolean;  // Admin can mark as recommended
}
...
const createPlaceholder = (color: string, text: string): string => {
...
function loadMoreComponents(): void {
    const grid = document.getElementById('componentsGrid');
    if (!grid) return;
...
function createComponentCard(component: Component): HTMLElement {
    ...
    // Add click events
    const viewBtn = card.querySelector('.view-component') as HTMLButtonElement;
    const purchaseBtn = card.querySelector('.purchase-component') as HTMLButtonElement;
    ...
    if (purchaseBtn) {
        purchaseBtn.addEventListener('click', (e: Event) => {
            e.stopPropagation();
...
function showComponentDetails(component: Component): void {
    ...
    const purchaseBtn = modalBody.querySelector('.purchase-detail') as HTMLButtonElement;
    if (purchaseBtn) {
...
function setupCodeTabs(container: HTMLElement): void {
    ...
    tabs.forEach((tab: Element) => {
        tab.addEventListener('click', () => {
            ...
            blocks.forEach((b: Element) => b.classList.remove('active'));
...
function purchaseComponent(component: Component): void {
    ...
    setTimeout(() => {
        const confirmBtn = document.getElementById('confirmPurchaseBtn') as HTMLButtonElement;
        const cancelBtn = document.getElementById('cancelPurchaseBtn') as HTMLButtonElement;
        const buyMoreBtn = document.getElementById('buyMorePointsBtn') as HTMLButtonElement;
...
function completePurchase(name: string, price: number, userPoints: number): void {
...
function openBuyPointsModal(): void {
    if (window.closeModal) {
        window.closeModal();
    }
...
window.completePurchase = function(name: string, price: number, userPoints: number) {
