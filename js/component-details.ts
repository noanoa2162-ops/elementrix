...
const componentDetailsContainer = document.getElementById('componentDetails') as HTMLDivElement;
const userPointsDisplay = document.getElementById('userPointsDisplay') as HTMLSpanElement;
const logoutBtn = document.getElementById('logoutBtn') as HTMLButtonElement;
const backToStoreBtn = document.getElementById('backToStoreBtn') as HTMLButtonElement;

// Get component ID from URL
const urlParams = new URLSearchParams(window.location.search);
const componentId: string | null = urlParams.get('id');
...
// Use global helper functions from load-components.ts
declare function getCategoryName(category: string): string;
declare function escapeHtml(text: string): string;

// Format code with proper line breaks
function formatCode(code: string, type: 'html' | 'css' | 'js'): string {
    if (type === 'css') {
...
const component: CodeComponent | undefined = storage.getApprovedComponents().find(c => c.id === componentId);
...
if (!component) {
    ...
    const canViewCode: boolean = hasPurchased || isOwner || isAdmin;
    ...
    const buyMoreBtn = document.getElementById('buyMoreBtn') as HTMLButtonElement | null;
    if (buyMoreBtn) {
    ...
    const purchaseBtnBottom = document.getElementById('purchaseBtnBottom') as HTMLButtonElement | null;
...
function createModalHeader(): string {
    return `
...
function createUploadOption(): string {
    return `
...
function createBuyOption(): string {
    return `
...
function createCloseButton(): string {
    return `
...
function addModalHoverEffects(): void {
    ...
    options.forEach(option => {
        option.addEventListener('mouseenter', () => {
            ...
            (option as HTMLElement).style.boxShadow = '0 12px 32px rgba(212, 175, 55, 0.2)';
        });
        option.addEventListener('mouseleave', () => {
            (option as HTMLElement).style.transform = 'translateY(0)';
            (option as HTMLElement).style.boxShadow = 'none';
        });
    });
}

// Buy More Points Modal (Component Details Page)
function openBuyMorePointsModal(): void {
...
// Copy code function
window.copyCode = function(type: string): void {
...
if (ratingStars.length > 0 && component && currentUser) {
    ...
    const userRating: number = storage.getUserRating(currentUser.id, component.id);
    
    // Highlight stars if already rated
    if (userRating > 0) {
        ratingStars.forEach((star, index) => {
            if (index < userRating) {
                (star as HTMLElement).classList.add('rated');
    ...
    ratingStars.forEach((star, index) => {
        star.addEventListener('mouseenter', () => {
            ratingStars.forEach((s, i) => {
                if (i <= index) {
                    ...
                    (s as HTMLElement).style.color = '#d1d5db';
        ...
        star.addEventListener('mouseleave', () => {
            const currentRating: number = userRating;
            ratingStars.forEach((s, i) => {
                if (i < currentRating) {
                    (s as HTMLElement).style.color = '#FFD700';
                } else {
        ...
        star.addEventListener('click', () => {
            ...
            ratingStars.forEach((s, i) => {
                (s as HTMLElement).classList.remove('rated');
                if (i < rating) {
                    (s as HTMLElement).style.color = '#FFD700';
                    (s as HTMLElement).classList.add('rated');
                    // Trigger bounce animation
                    setTimeout(() => {
                        (s as HTMLElement).style.transform = 'scale(1.5) rotate(360deg)';
                        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                if (updatedComponent) {
                    ...
                    if (ratingValue) ratingValue.textContent = updatedComponent.rating.toFixed(1);
                    if (ratingLabel) ratingLabel.textContent = `${updatedComponent.ratingsCount} דירוגים`;
                }
            }, 500);
        });
    });
